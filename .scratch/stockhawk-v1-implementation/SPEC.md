...
83. As the owner, I want a deny-by-default Tailscale Grant to authorize approved devices, so that StockHawk needs no application-local login, password, or session.
84. As the owner, I want Tailscale Serve to be StockHawk's sole ingress from home or away, so that no alternate LAN or public listener bypasses tailnet policy.
85. As the owner, I want loss of Tailscale to remove UI access without stopping local processing, so that the private boundary fails closed.
...
87. As the owner, I want PostgreSQL, the API, and worker supervised at boot, so that background processing recovers without manual repair.
...
- **Runtime topology:** Use one pinned Node.js 24 LTS/TypeScript/pnpm workspace with a React/Vite SPA, loopback-only Fastify API, separate collection worker, PostgreSQL 18, Drizzle plus reviewed SQL, pg-boss, Playwright, Pino, native `launchd`, and Tailscale Serve as the sole ingress. The SPA uses Tailwind CSS v4, locally owned shadcn/ui `base-nova` components over Base UI, stable TanStack Query v5, and the exact latest TanStack Table v9 beta. Do not add Docker Desktop, Caddy, another reverse proxy, Redis, or a separate search service.
...
- **Frontend foundation:** Initialize shadcn/ui with `style: "base-nova"`, Vite/React, TypeScript, CSS variables, neutral metadata, local aliases, and Lucide; use only its Base UI-backed variants and `@base-ui/react`. Own generated source in `apps/web`; do not add parallel Radix primitives or a one-consumer UI package. Tailwind's CSS-first theme must reproduce the locked artifact's exact semantic light/dark tokens rather than shadcn defaults.
- **Frontend versions:** Immediately before bootstrap, resolve and record the requested npm tags, then pin exact versions without ranges. The decision-date snapshot is shadcn `4.14.0`, Base UI `1.6.0`, Tailwind CSS/Vite plugin `4.3.3`, TanStack Query `5.101.4`, and TanStack Table/core `9.0.0-beta.55`. Table must use the current `beta` tag rather than untagged v8; keep core aligned exactly. Prerelease and stable upgrades are deliberate compatibility changes.
- **Frontend enforcement:** Adapt the tooling snapshot at commit `d60c74dcec2401125f912e710a30ca003bf6ed94`: type-aware Oxlint with TypeScript/React/React Compiler/TanStack Query/no-console rules, ESLint flat config with all compiler-aware Tailwind correctness rules, `prettier-plugin-tailwindcss`, Tailwind IntelliSense settings, and zero-warning gates. Ban arbitrary Tailwind values. Reusable locked-design choices become semantic theme tokens; one-off exact geometry uses component-owned CSS Modules or scoped custom properties. Keep strict peer checking; any narrow exact-version exception requires documented compatibility plus lint, typecheck, and runtime proof. Copy no ACERTUS assets, fonts, colors, components, broad whitelists, or repository-wide peer-check disable.
...
- **UI:** Reproduce the [locked owner design](../stockhawk-v1/design/DESIGN.md) in light and dark modes with Tailwind CSS v4, Base UI-backed shadcn components, TanStack Router, Query v5, and Table v9 beta. Its shell, exact tokens, typography, spacing, density, Search table, Health list/panels, semantic badges, controls, and responsive breakpoints are V1 acceptance criteria. Do not ship the old variant selector, side inspector, Storefront outline, card-per-result layout, or framework-default styling. Missing images never affect visibility or Health.
...
- **Security:** Bind Fastify and PostgreSQL to loopback and use Tailscale Serve as the only private inbound path. A deny-by-default Tailscale Grant and approved devices own authorization; Funnel, subnet routing, exit-node use, alternate LAN proxies, and public forwarding stay disabled. StockHawk deliberately has no application account, login, password, session, authentication cookie, or CSRF token. Retain exact-origin and Fetch-Metadata checks on mutations, SSRF redirect/address restrictions, log redaction, FileVault, and the macOS firewall. Do not add application authentication back without a new owner-approved architecture decision.
- **Operations:** `launchd` supervises database, web, worker, and backup jobs. Worker/browser/Tailscale/internet failures must not destroy local search truth. Tailscale loss makes the UI unavailable while background services continue; access resumes only when the supported Tailscale client and Serve return. Daily validated custom-format dumps retain seven generations; weekly clean restores prove recovery.
...
- Verify the pinned Table v9 beta and Query v5 integration through type checks and behavioral contracts for server-managed sorting/filtering/keyset pagination, URL state, cache keys, optimistic updates, rollback, and reconciliation. Use Table v9's `useTable`/feature APIs and official shadcn Base UI example, not shadcn's still-v8 Data Table recipe. A beta upgrade reruns the complete UI contract and visual suite.
- Run type-aware Oxlint, Tailwind ESLint with zero warnings, Prettier Tailwind-order checks, and unit tests for accepted/rejected class examples. Fail arbitrary, unknown, contradictory, unnecessarily arbitrary, and incorrectly ordered utilities; prove semantic tokens compile in both themes.
...
- Run representative 100,000-Offer search and UI performance on the actual production topology. Require the first page over the private Tailscale path within two seconds and search/filter/view/page updates within 500 milliseconds; record distributions and query plans.
...
- Rehearse on the actual Mac: clean provision, migration, load, worker/database/browser/network/Tailscale failures, no-login background-service reboot, post-login Serve recovery, proof of Tailscale-only ingress and deny-by-default device authorization, residential egress, exact-origin/Fetch-Metadata and SSRF controls, log redaction, backup, and restore.
...
- Public cloud hosting, hosted/remote task caching, public exposure, Tailscale Funnel, subnet routing, exit-node crawling, application-local accounts/passwords/sessions, multi-user accounts, native mobile apps, or a public API.
...
- Visual redesign, unapproved baseline drift, alternative primitive/CSS systems, or design-system substitution outside the locked owner design.
...
- Frontend bootstrap follows the [primary-source stack snapshot](../stockhawk-v1/research/frontend-stack-primary-sources.md) and the StockHawk-specific [frontend tooling reference](../stockhawk-v1/research/frontend-tooling-reference.md).
