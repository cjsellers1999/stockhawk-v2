...
- **Runtime topology:** Use one pinned Node.js 24 LTS/TypeScript/pnpm workspace with a React/Vite SPA, Fastify API, separate collection worker, PostgreSQL 18, Drizzle plus reviewed SQL, pg-boss, Playwright, Pino, native `launchd`, Tailscale Serve, and Caddy LAN fallback. The SPA uses Tailwind CSS v4, locally owned shadcn/ui `base-nova` components over Base UI, stable TanStack Query v5, and the exact latest TanStack Table v9 beta. Do not add Docker Desktop, Redis, or a separate search service.
