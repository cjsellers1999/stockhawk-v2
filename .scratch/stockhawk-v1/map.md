...
- V1 is a private web application available only through Tailscale Serve to devices allowed by a deny-by-default Grant. It has no application-local authentication, alternate LAN ingress, or public exposure. Purchasing is a manual handoff to the retailer.
- Search behavior decisions remain captured in the [`Search prototype verdict`](../search-ui-prototype/NOTES.md). The complete visual and interaction authority is the [locked owner design](design/DESIGN.md), preserving the explicitly supplied HTML verbatim at SHA-256 `1b03f2a6bbe5b0d4cad972901c100057b658c1375540a283f2f90821a85cb23a`.
...
- [Prototype the search and Health Page experience](issues/08-prototype-search-and-health-experience.md) — Implement the owner-approved locked Search and Health design with Tailwind CSS v4, locally owned shadcn/ui `base-nova` components over Base UI, exact-pinned stable TanStack Query v5, and exact-pinned latest TanStack Table v9 beta; retain the dense Offer table, operational Health list, exact light/dark tokens, and no card-per-result layout.
- [Choose the local stack and deployment topology](issues/09-choose-the-local-stack-and-deployment-topology.md) — Run a coarse pnpm/Turborepo workspace as native launchd-supervised Node/React/Fastify and collection processes over PostgreSQL, Drizzle, and pg-boss; validate untrusted ingress through shared Zod contracts; use Tailscale Serve as the sole private ingress with deny-by-default device authorization, enforced optimistic commands, and restore-tested daily backups.
...
- Public cloud hosting, Tailscale Funnel or other public internet exposure, alternate LAN ingress, application-local authentication, and native mobile applications.
