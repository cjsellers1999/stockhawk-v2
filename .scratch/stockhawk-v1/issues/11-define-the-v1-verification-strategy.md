...
- Prove the sole private-access boundary: a device allowed by the deny-by-default Tailscale Grant can use Serve; unapproved tailnet, direct-LAN, and public clients cannot; Funnel/exit/subnet/public forwarding stay disabled; and neither API nor PostgreSQL is directly reachable. No application login/session is part of this boundary.
- Reboot with no macOS login and prove PostgreSQL, API, and worker restore background operation; then log in normally and prove persistent Tailscale Serve returns without reconfiguration. Tailscale failure must remove UI access while leaving collection and durable processing intact.
...
Before full rollout and after material topology or migration changes, the target Mac runs a representative rehearsal: clean provision/migration, 100,000-Offer search and scheduler load, accepted query plans and latency distributions, process and network failure injection, worker kill before/after commit, no-login background-service recovery through `launchd`, Tailscale Serve recovery after normal login, proof that no alternate ingress exists, loopback-only API/database, Grant-approved-versus-unapproved/public access, exact-Origin/Fetch-Metadata and SSRF/log-redaction tests, and proof that retailer traffic still uses the home residential IP and broker.
