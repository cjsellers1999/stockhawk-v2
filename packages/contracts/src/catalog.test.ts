...
describe("catalog contracts", () => {
  ...
  it("rejects deceptive or cross-Storefront evidence and handoff URLs", () => {
    const command = validObservationBatch();
    ...
    expect(() =>
      commitObservationBatchCommandSchema.parse({
        ...
        listing: {
          ...command.listing,
          purchaseUrl: "https://unrelated.example/products/sky-dragon-medium",
        },
      }),
    ).toThrow(/Storefront origin/i);
    expect(() =>
      commitObservationBatchCommandSchema.parse({
        ...command,
        evidence: {
          ...command.evidence,
          sourceUrl: "https://unrelated.example/products/sky-dragon-medium",
        },
      }),
    ).toThrow(/Storefront origin/i);
  });

  it("normalizes trusted HTTP URLs before persistence", () => {
    const command = validObservationBatch();
    const normalized = commitObservationBatchCommandSchema.parse({
      ...command,
      evidence: {
        ...command.evidence,
        sourceUrl:
          "HTTPS://LILTULIPS.COM:443/products/../products/sky-dragon-medium",
      },
      storefront: {
        ...command.storefront,
        origin: "HTTPS://LILTULIPS.COM:443/",
      },
    });

    expect(normalized.evidence.sourceUrl).toBe(
      "https://liltulips.com/products/sky-dragon-medium",
    );
    expect(normalized.storefront.origin).toBe("https://liltulips.com");
  });
